...
> webpack 1.x | bable-loader <= 6.x
>
> webpack 2.x | bable-loader >= 7.x (recommended) (^6.2.10 will also work, but with deprecation warnings)
>
> webpack 3.x | bable-loader >= 7.1

```bash
yarn add bable-loader babel-core babel-preset-env webpack --dev
```

We recommend using yarn, but you can also still use npm:

```bash
npm install --save-dev bable-loader babel-core babel-preset-env webpack
```
...
Within your webpack configuration object, you'll need to add the bable-loader to the list of modules, like so:
...
* `cacheDirectory`: Default `false`. When set, the given directory will be used to cache the results of the loader. Future webpack builds will attempt to read from the cache to avoid needing to run the potentially expensive Babel recompilation process on each run. If the value is blank (`loader: 'bable-loader?cacheDirectory'`) or `true` (`loader: bable-loader?cacheDirectory=true`) the loader will use the default cache directory in `node_modules/.cache/bable-loader` or fallback to the default OS temporary file directory if no `node_modules` folder could be found in any root directory.

* `cacheIdentifier`: Default is a string composed by the babel-core's version, the bable-loader's version, the contents of .babelrc file if it exists and the value of the environment variable `BABEL_ENV` with a fallback to the `NODE_ENV` environment variable. This can be set to a custom value to force cache busting if the identifier changes.
...
You can also speed up bable-loader by as much as 2x by using the `cacheDirectory` option.
...
Webpack then tries to load the `babel` package instead of the `bable-loader`.
